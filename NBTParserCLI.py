...
#%% Enrich NBT data by applying functions to certain tags
...
modification_list = [
    ...
    {"apply_to": "custom_name_plaintext", "new_val": "custom_name_plaintext",  "func": lambda x: (x[1:] if x[0] == '.' else x).strip()},
...
#%%
# Place all container/bundle contents in seperate records
flattened_items = NBTProcessor.flatten_items(enhanced_items)
# Transform the items into a flat, printable format
beautified_items = NBTBeautifier.beautify_item(flattened_items)
# Deduplicate
unique_beautified_items = NBTProcessor.drop_duplicates(beautified_items)
#remove all vanilla items at the top level
...
#%% Apply manual modifications which could not be automated
